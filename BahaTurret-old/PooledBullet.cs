...
namespace BahaTurret
{
	public class PooledBullet : MonoBehaviour
	{
		...
		void FixedUpdate()
		{
			...
			if(tracerLength == 0)
			{
                p1 = transform.position+(currentVelocity * 1.35f * TimeWarp.fixedDeltaTime/TimeWarp.CurrentRate)-(FlightGlobals.ActiveVessel.rb_velocity*TimeWarp.fixedDeltaTime);
				...
				if(Physics.Raycast(ray, out hit, dist, 557057))
				{
					...
					if(hitPart == null || (hitPart!=null && hitPart.vessel!=sourceVessel))
					{
						...
						else
						{
							...
							{
                                ExplosionFX.CreateExplosion(hit.point, radius, blastPower, sourceVessel, currentVelocity.normalized, explModelPath, explSoundPath, true);
							}
			...
			{
				//detonate
                ExplosionFX.CreateExplosion(transform.position, radius, blastPower, sourceVessel, currentVelocity.normalized, explModelPath, explSoundPath, true);
				//GameObject.Destroy(gameObject); //destroy bullet on collision
				KillBullet();
				return;
			}
